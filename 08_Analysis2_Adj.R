...
output_dir <- here("MCMCSamples", "Analysis2", "Adj", "model1")
...
output_dir <- here("MCMCSamples", "Analysis2", "Adj", "model2")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("MCMCSamples", "Analysis2", "Adj", "model3")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("MCMCSamples", "Analysis2", "Adj", "model4")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_fig_dir <- here("Figures", "Analysis2", "Adj")
...
gp <- fit3_pred %>% subset(category=="ber") %>% ggplot(aes(x=vcb))+
  ...
  theme_classic()+
  scale_x_continuous(limits=c(0,62), breaks=seq(0,62,by=15))+
  scale_y_continuous(limits=c(0,1))+
  labs(y=expression(paste("Mean proportion of producing scale errors (", italic(theta), ")", sep="")), x="Vocabulary size\n(Adjectives)")+
...
print(gp)
ggsave(file = here("Figures", "Analysis2", "Adj", "Figure_Adj_Ber.png"), plot=gp, dpi=350, width=2.5, height=4.6)  
...
gp <- fit3_pred %>% subset(category=="poi") %>% ggplot(aes(x=vcb))+
  ...
  theme_classic()+
  scale_x_continuous(limits=c(0,62), breaks=seq(0,62,by=15))+
  scale_y_continuous(limits=c(0,11.5), breaks=0:11)+
  labs(y=expression(paste("Mean number of scale errors (", italic(lambda), ")", sep="")), x="Vocabulary size\n(Adjectives)")+
  theme(axis.ticks=element_line(color = "black"),
        axis.text.x=element_text(size=12, color="black", angle=0, hjust=0.5),
        axis.text.y=element_text(size=12, color="black"),
        axis.title=element_text(size=14, color="black"),
        ...
        legend.position="none") 
print(gp)
ggsave(file = here("Figures", "Analysis2", "Adj", "Figure_Adj_Poi.png"), plot=gp, dpi=350, width=2.5, height=4.6)  

# Overall ZIP
d %>% group_by(vcb) %>% count(se_sum) %>% ungroup() %>% 
  summarise(Max = max(n), MIN = min(n))   # Max = 13, MIN = 1
...
gp <- d %>%  group_by(vcb) %>% count(se_sum) %>% ungroup() %>% 
  ...
  scale_x_continuous(limits=c(0,62), breaks=seq(0,62,by=15))+
  scale_y_continuous(limits=c(0,11.5), breaks=0:11)+
  labs(y=expression(paste("Mean number of scale errors (ZIP model)", sep="")), x="Vocabulary size\n(Adjectives)")+
...
print(gp)
ggsave(file = here("Figures", "Analysis2", "Adj", "Figure_Adj_ZIP.png"), plot=gp, dpi=350, width=3.0, height=4.6)  
...
gp <- d %>% ggplot()+
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.justification=c(1,0))
print(gp)
ggsave(file = here("Figures", "Analysis2", "Adj", "Figure_fit3_PostPredictiveCheck.png"), plot=gp, dpi=350, width=3.0, height=4.4)  
...
fit3_agediff <- ggs(fit3) %>%
  ...
  mutate(category = str_sub(Parameter, start=8, end=10),
         ageid = as.numeric(str_sub(Parameter, start=12, end=-1))) %>%
  left_join(d_age, by="ageid") %>%
  subset(vcb==0 | vcb==12 | vcb==15 | vcb==62) %>%
  select(-Parameter, -ageid, -vcb_z) %>%
  pivot_wider(names_from = c(category, vcb), values_from = value) %>%
  mutate(diff_Ber_12_minus_0   = ber_12 - ber_0,
         diff_Ber_12_minus_62 = ber_12 - ber_62,
         diff_Poi_62_miuns_0  = poi_62 - poi_0,
         diff_All_15_minus_0   = all_15 - all_0,
         diff_All_15_minus_62 = all_15 - all_62) %>%
