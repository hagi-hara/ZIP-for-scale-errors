...
output_dir <- here("MCMCSamples", "Analysis2", "Verb", "model1")
...
output_dir <- here("MCMCSamples", "Analysis2", "Verb", "model2")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("MCMCSamples", "Analysis2", "Verb", "model3")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("MCMCSamples", "Analysis2", "Verb", "model4")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_fig_dir <- here("Figures", "Analysis2", "Verb")
...
gp <- fit3_pred %>% subset(category=="ber") %>% ggplot(aes(x=vcb))+
  ...
  theme_classic()+
  scale_x_continuous(limits=c(0,102), breaks=seq(0,100,by=20))+
...
print(gp)
ggsave(file = here("Figures", "Analysis2", "Verb", "Figure_Verb_Ber.png"), plot=gp, dpi=350, width=2.5, height=4.6)  
...
gp <- fit3_pred %>% subset(category=="poi") %>% ggplot(aes(x=vcb))+
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.position="none") 
print(gp)
ggsave(file = here("Figures", "Analysis2", "Verb", "Figure_Verb_Poi.png"), plot=gp, dpi=350, width=2.5, height=4.6)  
...
print(gp)
ggsave(file = here("Figures", "Analysis2", "Verb", "Figure_Verb_ZIP.png"), plot=gp, dpi=350, width=3.0, height=4.6)  
...
gp <- d %>% ggplot()+
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.justification=c(1,0))
print(gp)
ggsave(file = here("Figures", "Analysis2", "Verb", "Figure_fit3_PostPredictiveCheck.png"), plot=gp, dpi=350, width=3.0, height=4.4)  
...
fit3_agediff <- ggs(fit3) %>%
  ...
  mutate(category = str_sub(Parameter, start=8, end=10),
         ageid = as.numeric(str_sub(Parameter, start=12, end=-1))) %>%
  left_join(d_age, by="ageid") %>%
  subset(vcb==0 | vcb==30 | vcb==20 | vcb==102) %>%
  select(-Parameter, -ageid, -vcb_z) %>%
  pivot_wider(names_from = c(category, vcb), values_from = value) %>%
  mutate(diff_Ber_20_minus_0   = ber_20 - ber_0,
         diff_Ber_20_minus_102 = ber_20 - ber_102,
         diff_Poi_102_miuns_0  = poi_102 - poi_0,
         diff_All_30_minus_0   = all_30 - all_0,
         diff_All_30_minus_102 = all_30 - all_102) %>%
...
################################################
# Model 5. B-splines with 5 base functions
################################################
