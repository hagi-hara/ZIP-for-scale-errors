...
vcb_pred <- d %>% select(starts_with("vcball")) %>% distinct() %>% arrange(vcball)
...
dlist_vcb <- list(N = nrow(d),
                  ...
                  age_pred = vcb_pred$vcball_z,
                  age2_pred = vcb_pred$vcball_z^2)


############################################################
##### Vocabulary size as the developmental index ###########
############################################################
...
output_dir <- here("MCMCSamples", "Analysis2", "TotalVcb", "model1")
...
output_dir <- here("MCMCSamples", "Analysis2", "TotalVcb", "model2")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("MCMCSamples", "Analysis2", "TotalVcb", "model3")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_dir <- here("MCMCSamples", "Analysis2", "TotalVcb", "model4")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
output_fig_dir <- here("Figures", "Analysis2", "TotalVcb")
...
# Logistic part
gp <- fit3_pred %>% subset(category=="ber") %>% ggplot(aes(x=vcball))+
...
print(gp)
ggsave(file = here("Figures", "Analysis2", "TotalVcb", "Figure_4a.png"), plot=gp, dpi=350, width=2.5, height=4.4)  

# Count part
gp <- fit3_pred %>% subset(category=="poi") %>% ggplot(aes(x=vcball))+
  geom_line(aes(y=MED, group=category), color="#D95F02", lwd=2)+
  geom_ribbon(aes(ymax=upr, ymin=lwr), fill="#D95F02", alpha=0.25)+
  theme_classic()+
  scale_x_continuous(limits=c(0,700), breaks=seq(0,700,by=150))+
  scale_y_continuous(limits=c(0,8.55), breaks=0:9)+
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.position="none") 
print(gp)
ggsave(file = here("Figures", "Analysis2", "TotalVcb", "Figure_4b.png"), plot=gp, dpi=350, width=2.5, height=4.4)  
...
print(gp)
ggsave(file = here("Figures", "Analysis2", "TotalVcb", "Figure_4c.png"), plot=gp, dpi=350, width=3.0, height=4.4)  
...
d %>% subset(se_sum >= 0 & se_sum <= 5) %>% nrow() / nrow(d) # 98.9% of the actual data are within 95% of post predictive distribution, [0,5]
...
gp <- d %>% ggplot()+
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.justification=c(1,0))
print(gp)
ggsave(file = here("Figures", "Analysis2", "TotalVcb", "Figure_fit3_PostPredictiveCheck.png"), plot=gp, dpi=350, width=3.0, height=4.4)  
...
fit3_agediff <- ggs(fit3) %>% 
  ...
  mutate(category = str_sub(Parameter, start=8, end=10),
         ageid = as.numeric(str_sub(Parameter, start=12, end=-1))) %>% 
  left_join(d_age, by="ageid") %>% 
  subset(vcball==0 | vcball==141 | vcball==183 | vcball==700) %>% 
  select(-Parameter, -ageid, -vcball_z) %>% 
  pivot_wider(names_from = c(category, vcball), values_from = value) %>% 
  mutate(diff_Ber_141_minus_0 = ber_141 - ber_0,
         diff_Ber_141_minus_700 = ber_141 - ber_700,
         diff_Poi_700_miuns_0 = poi_700 - poi_0,
         diff_All_183_minus_0 = all_183 - all_0,
         diff_All_183_minus_700 = all_183 - all_700) %>% 
...
############################################################
##### Age in months as the developmental index #############
############################################################
...
fit5 <- model5$sample(data = dlist_age,
                      chains = 4,
                      iter_warmup = warmup_set,
                      iter_sampling = iter_set,
                      adapt_delta = 0.995, 
...
output_dir <- here("MCMCSamples", "Analysis2", "TotalVcb", "model5")
if (!dir.exists(output_dir)){
  dir.create(output_dir, recursive = TRUE)
}
...
rbind(waic1$estimates,
  ...
  cbind(Formula = rep(c("bf(se_sum ~ vcb + vcb^2, zi ~ vcb + vcb^2)",
                        ...
                        "bf(se_sum ~ vcb, zi ~ vcb)",
                        "bf(se_sum ~ age, zi ~ age + age^2)"), each = 3)) %>%
