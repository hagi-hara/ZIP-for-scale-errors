...
transformed parameters{
  ...
  for(n in 1:N) {
    theta[n] = inv_logit(b_beta0 + b_beta1 * age[n] + b_beta3 * country[n] + b_beta4 * session[n]);
    lambda[n] = exp(p_beta0 + p_beta1 * age[n] + p_beta2 * age2[n] + p_beta3 * country[n] + p_beta4 * session[n]);
  }
}
...
generated quantities {
  ...
  for(i in 1:N_age_pred) {
    Y_pred_ber[i] = inv_logit(b_beta0 + b_beta1 * age_pred[i] + b_beta3 * 0.5);
