...
transformed parameters{
  ...
  for(n in 1:N) {
    theta[n] = inv_logit(b_beta0 + b_beta1 * age[n] + b_beta2 * age2[n] + b_beta3 * countUK[n] + b_beta4 * session[n] + b_beta5 * obj[n] + b_beta6 * dur[n] + b_beta7 * gender[n]);
    lambda[n] = exp(p_beta0 + p_beta1 * age[n] + p_beta2 * age2[n] + p_beta3 * countUK[n] + p_beta4 * session[n] + p_beta5 * obj[n] + p_beta6 * dur[n] + p_beta7 * gender[n]);
  }
}
...
generated quantities {
  ...
  for(i in 1:N_age_pred) {
    Y_pred_ber[i] = inv_logit(b_beta0 + b_beta1 * age_pred[i] + b_beta2 * age2_pred[i] + b_beta3 * 1/2 + b_beta7 * 1/2);
    Y_pred_poi[i] = exp(p_beta0 + p_beta1 * age_pred[i] + p_beta2 * age2_pred[i] + p_beta3 * 1/2 + p_beta7 * 1/2);
