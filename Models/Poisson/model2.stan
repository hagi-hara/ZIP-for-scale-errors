...
model {
  Y ~ poisson_log(p_beta0 + p_beta1 * age + p_beta3 * country + p_beta4 * session);
...
generated quantities {
  ...
  for(i in 1:N) {
    Y_dist[i] = poisson_log_rng(p_beta0 + p_beta1 * age[i] + p_beta3 * country[i] + p_beta4 * session[i]);
  }
  
  for(i in 1:N){
    log_lik[i] = poisson_log_lpmf(Y[i] | p_beta0 + p_beta1 * age[i] + p_beta3 * country[i] + p_beta4 * session[i]);
  }
}
