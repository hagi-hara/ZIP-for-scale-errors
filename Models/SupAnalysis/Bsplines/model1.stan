...
transformed parameters{
  ...
  theta = inv_logit(to_vector(b_beta*B) + b_beta_countUK * countUK + b_beta_countUS * countUS + b_beta_session * session + b_beta_obj * obj + b_beta_dur * dur + b_beta_gender * gender);
  lambda = exp(to_vector(p_beta*B) + p_beta_countUK * countUK + p_beta_countUS * countUS + p_beta_session * session + p_beta_obj * obj + p_beta_dur * dur + p_beta_gender * gender);
}
...
model {
  ...
  b_beta_countUS ~ student_t(3, 0, 1);
...
generated quantities {
  ...
  Y_pred_ber = inv_logit(to_vector(b_beta*B_pred) + b_beta_countUK * 1/3 + b_beta_countUS * 1/3 + b_beta_gender * 1/2);
