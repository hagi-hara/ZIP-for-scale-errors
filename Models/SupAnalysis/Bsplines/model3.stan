...
transformed parameters{
  ...
  theta = inv_logit(to_vector(b_beta*B) + b_beta_session * session);
  lambda = exp(to_vector(p_beta*B) + p_beta_session * session + p_beta_obj * obj);
}
...
generated quantities {
  ...
  Y_pred_ber = inv_logit(to_vector(b_beta*B_pred));
