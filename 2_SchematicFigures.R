...
print(gp)
ggsave(file = here("Figures", "Figure_1_1.png"), plot=gp, dpi=350, width=6.2, height=4.2)    
...
gp <- data.frame(x = x, y = -18*(x-0.5)^2+6+rnorm(x,0,1)) %>% 
  ...
  theme(axis.text.y=element_blank(),
        ...
        axis.line=element_line(color="black", size=1))
print(gp)
ggsave(file = here("Figures", "Figure_1_2.png"), plot=gp, dpi=350, width=6.2, height=4.2)    
...
print(gp)
ggsave(file = here("Figures", "Figure_1_3.png"), plot=gp, dpi=350, width=6.2, height=2.2)
...
gp <- as.data.frame(t(w*bsMat)) %>% cbind(x) %>% 
  ...
  theme(axis.text.y=element_blank(),
        ...
        legend.position="none")
print(gp)
ggsave(file = here("Figures", "Figure_1_4.png"), plot=gp, dpi=350, width=6.2, height=4.2)
...
gp <- d.sim %>% mutate(Bernoulli = as.factor(Bernoulli)) %>% 
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.position="none")
print(gp)
ggsave(file = here("Figures", "Figure_2.png"), plot=gp, dpi=350, width=4.2, height=4.2)
